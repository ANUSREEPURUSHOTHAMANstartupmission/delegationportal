<nav :class="{ 'left-64 -right-64 md:left-0 md:right-0 ': open, 'left-0 right-0 md:left-64 ': !(open) }" class=" fixed flex flex-row flex-nowrap items-center justify-between mt-0 py-2 left-0 md:left-64 right-0 px-6 bg-white dark:bg-gray-800 shadow-sm transition-all duration-500 ease-in-out z-50" id="desktop-menu">
    ...
    <ul class="flex ml-auto mt-2">
      <li x-data="{ open: false }" class="relative">
        <a href="javascript:;" class="px-3 flex text-sm rounded-full focus:outline-none" id="user-menu-button" @click="open = ! open">
          <div class="relative">
            <div class="h-10 w-10 rounded-full border border-gray-700 bg-gray-700 items-center justify-center text-center flex ">
              <div class="text-white text-center items-center justify-center text-lg">
          ...
          <span class="hidden md:block ml-1 self-center dark:text-white">
            {{auth()->user()->name}}
          </span>
        </a>
